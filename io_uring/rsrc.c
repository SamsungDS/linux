...
struct io_regbuf_dma {
	...
	struct io_dmabuf_map		*cached_map;
};
...
void io_drop_dmabuf_node(struct io_rsrc_node *node)
{
	struct io_mapped_ubuf *imu;
	...
	if (!IS_ENABLED(CONFIG_DMABUF_TOKEN))
		return;
	if (WARN_ON_ONCE(node->type != IORING_RSRC_BUFFER))
		return;
	...
	io_dmabuf_map_drop(db->cached_map);
}
