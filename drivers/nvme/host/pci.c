				   ...
				   struct nvme_cdq_mgmt * cdq_mgmt)
{
	...
	ret = nvme_pci_cdq_entry_alloc(dev, &cdq,
				       ...
				       cdq_mgmt->cdq_create.entry_nbyte);
	if (ret)
		return ret;
	...
	ret =  __nvme_submit_sync_cmd(dev->ctrl.admin_q, &c, &result, NULL, 0, NVME_QID_ANY, 0, 0);
	if (ret)
		return ret;
					      ...
					      struct cdq_nvme_queue* cdq, void* priv)
{
	...
	void *entries_start;
	size_t copy_nentry = min(cdq->entry_nr - init_entry, (const u32)tx_nentry);
