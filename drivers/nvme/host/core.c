						   ...
						   struct nvme_command *cmnd)
{
	...
	struct bio *bio;
	u64 dlba = 0, slba = 0, nlba = 0;
	u16 nr_range = 1, control = 0, index = BLK_COPY_DST_BIO_INDEX;
	...
	__rq_for_each_bio(bio, req) {
		if (index == BLK_COPY_DST_BIO_INDEX) {
			dlba = nvme_sect_to_lba(ns->head,
						bio->bi_iter.bi_sector);
			nlba = bio->bi_iter.bi_size >> ns->head->lba_shift;
		} else {
			slba = nvme_sect_to_lba(ns->head,
						bio->bi_iter.bi_sector);
			if (nlba != bio->bi_iter.bi_size >> ns->head->lba_shift)
				return BLK_STS_IOERR;
		}
		index++;
	}
	...
	cmnd->copy.sdlba = cpu_to_le64(dlba);
	...
	range[0].nlb = cpu_to_le16(nlba - 1);
